...
bool UChartImporterCytus::ImportFromFile(FString FilePath, TArray<class UInstruction *>& ImportResult)
{
	...
	uint32 CurrentLine = 4; //Ignore BPM,PAGE_SHIFT and PAGE_SIZE
	for (; CurrentLine < FileContents.Num(); CurrentLine++)
	{
		...
		if (RecordParts.Num()==0||FileContents[CurrentLine]!="NOTE")
			break;
	...
	for (int i=0;i<Notes.Num();i++)
	{
		if (Notes[i].bIsLinked)
			continue;
		URhythmTap* CurNote = Cast<URhythmTap>(InstructionManager->GenInstruction("RhythmTap", Notes[i].Time));
		...
		CurNote->OnInstructionLoaded_Editor_Implementation(Args,FEditorExtraInfo());
	...
	for (int i=0;i<LinkedNotes.Num();i++)
	{
		...
		URhythmSlide* CurNote = Cast<URhythmSlide>(InstructionManager->GenInstruction("RhythmSlide", Notes[LinkedNotes[i][0]].Time));
		FBlueprintJsonObject Args;
