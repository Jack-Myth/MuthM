...
UUIProvider* UUIProvider::Get(const UObject* WorldContextObj)
{
	...
	if (!GameInstance->UIProvider)
		GameInstance->UIProvider=NewObject<UUIProvider>(GameInstance);
	return GameInstance->UIProvider;
}
...
TSubclassOf<UMusicImportationUIBase> UUIProvider::GetMusicImportationUI() 
{ 
	if(MusicImportationUI.IsBound()) 
	{ 
		TSubclassOf<UMusicImportationUIBase> _uclass; 
		MusicImportationUI.Execute(_uclass);
		return _uclass;
	} 
	else
	{
